...
//    }
    ...
    public ResponseUtils selectByLike(@RequestParam("keyword") String keyword) {
        try {
            List<ArticleList> articleLists = articleListService.selectByLike(keyword);
            if (articleLists != null && !articleLists.isEmpty()) {
                return new ResponseUtils(200, "查询成功", articleLists);
            } else {
                return new ResponseUtils(404, "未找到相关文章"); // 更准确的状态码应该是 404 而不是 400
            }
        } catch (Exception e) {
            e.printStackTrace();
...
//        System.out.println(111);
    ...
    public ResponseUtils update(@RequestBody ArticleListVO articleListVO){
        try {
            //我们获取到了前端的四个参数之后，下一步应该是干嘛? 主要是为了修改
            //操作数据库，但是写到这里发现少了东西?接口
            int result = articleListService.update(articleListVO);
            if (result == 1){
                return new ResponseUtils(200,"修改成功");
            }else {
                return new ResponseUtils(500,"修改失败");
     ...
     * 查询单个文章
     */
    @GetMapping("/selectById")  // 改为 GetMapping
    public ResponseUtils selectById(@RequestParam Long id) {  // 改为 @RequestParam 接收参数
        try {
            if (id == null) {
                return new ResponseUtils(400, "ID不能为空");
            }

            ArticleList articleList = articleListService.selectById(id);
            if (articleList != null) {
                return new ResponseUtils(200, "查询成功", articleList);
            } else {
                return new ResponseUtils(400, "查询失败");
            }
        } catch (Exception e) {
            e.printStackTrace();
            return new ResponseUtils(500, "查询异常：" + e.getMessage());
        }
    }
